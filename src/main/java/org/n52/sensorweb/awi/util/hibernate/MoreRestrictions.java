...
public class MoreRestrictions {
    ...
    public static Optional<? extends Criterion> and(Optional<? extends Criterion>... criteria) {
        Conjunction conjunction = Arrays.stream(criteria)
                ...
                .map(Optional::get).collect(toConjunction());
        return Optional.of(conjunction)
                .filter(MoreRestrictions::hasConditions);
    }
    ...
    public static Optional<? extends Criterion> or(Optional<? extends Criterion>... criteria) {
        Disjunction disjunction = Arrays.stream(criteria)
                .filter(Optional::isPresent)
                .map(Optional::get).collect(toDisjunction());
